...
\textbf{Feature Ablation: Track-Geometry Features.}
We evaluated LR-instant and RF-instant with all absolute track-geometry features removed
(NO-TRACK condition: X, Y, Z GPS coordinates \emph{and} elevation delta dropped, leaving
eight purely kinematic/powertrain features). Mean AUC across LOCO folds \emph{improved} to
$0.969$ for LR (FULL: $0.963$, $\Delta = +0.007$) and $0.971$ for RF
(FULL: $0.959$, $\Delta = +0.012$).
Removing every channel that could encode circuit geometry thus increases performance,
confirming that the results are driven entirely by physics-based kinematics
(speed, gear, throttle, brake, acceleration, and their derived quantities) with no reliance
on circuit-identity leakage through position or elevation.
...
CausalTrans.    & 0.68 & $+$1.1 & 1.33 & 0.70 & 0.102 \\
CausalGRU       & 0.61 & $+$0.4 & 1.47 & 0.65 & 0.105 \\
\midrule
...
\begin{itemize}
  ...
  \item \textbf{Track-geometry circuit identity leakage.} Features derived from
        absolute track position and elevation could in principle encode circuit identity.
        A full no-track ablation (§\ref{sec:results}, NO-TRACK condition) removing
        X, Y, Z \emph{and} elevation delta \emph{improved} LR AUC by $+0.007$ and RF AUC
        by $+0.012$, confirming that these channels contribute noise rather than signal and
        that results are attributable to physics-based kinematics features. Cumulative lap
        distance is not among the 12 model features, so it cannot contribute leakage.
