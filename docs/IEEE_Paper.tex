...
%  Authors: Anas Norani, Hanan Majeed, Maha Mohsin
%  SEECS, NUST — June 2026
% ============================================================
...
% ── Title ───────────────────────────────────────────────────
\title{Anticipatory Active-Aerodynamic Mode Prediction in Formula~1 Racing:\\
...
% ============================================================
\begin{abstract}
The 2026 Formula~1 Technical Regulations mandated active aerodynamic systems that
physically switch wing configurations between a high-downforce \emph{Z-Mode} and a
low-drag \emph{X-Mode}.
We study a telemetry-derived proxy for the activation condition
($v \geq 240$\,km/h and gear~$\geq 6$) as a tractable surrogate for the
circuit-specific activation logic teams use in practice.
Because the physical transition is not instantaneous, a predictive system that
anticipates the required mode one second in advance can eliminate the actuation
latency and improve vehicle performance.
...
\emph{Phase~2} trains eight models — spanning Logistic Regression, Random Forest
(with and without lag features), CausalLSTM, CausalGRU, Temporal Convolutional
Network (TCN), CNN1D, and a CausalTransformer — for one-second-ahead anticipatory
...
which Integrated Gradients associate with \emph{circuit speed memorisation}: diffuse
reliance on accumulated speed-trajectory dynamics in the recurrent hidden state.
...
% ── Keywords ────────────────────────────────────────────────
\begin{IEEEkeywords}
active aerodynamics, Formula~1 telemetry, time-series classification,
anomaly detection, causal transformer, leave-one-circuit-out,
cross-domain generalisation, integrated gradients
\end{IEEEkeywords}
...
The 2026 FIA Technical Regulations (Article~3.10) introduced mandatory active
aerodynamic systems: wings that physically change their angle of attack during a lap,
alternating between a maximum-downforce configuration for corners (\emph{Z-Mode}) and
a minimum-drag configuration for straights (\emph{X-Mode})~\cite{ref_fia2026}.
The switch activates when two simultaneous conditions are satisfied:
vehicle speed~$\geq 240$\,km/h \textbf{and} gear~$\geq 6$.
...
Because active wing actuation is mechanical/hydraulic, the physical transition
requires approximately 100--300\,ms~\cite{ref_cfd}.
At racing speeds ($\sim$370\,km/h $\approx$ 103\,m/s), this delay covers roughly
10--30\,m of track during which the car operates in a suboptimal aerodynamic
configuration.
...
can begin actuation early, ensuring the wing arrives at the correct angle precisely
when the threshold is reached.
At 330\,km/h ($\approx 91.7$\,m/s), a 1-second prediction horizon corresponds to a
91\,m lead time — more than sufficient for a 150--200\,ms hydraulic actuation cycle~\cite{ref_cfd},
eliminating the aerodynamic deficit currently incurred during the transition delay.
...
but to our knowledge no prior work has addressed the anticipatory prediction of
...
time-series variants: Informer~\cite{ref_informer}, PatchTST~\cite{ref_patchtst}, and
FEDformer~\cite{ref_fedformer}.
...
circuit as a distinct domain; to our knowledge this is the first application of LOCO
evaluation to motorsport aerodynamic state prediction.
...
For each race session, lap-by-lap telemetry was extracted from a leading
competitor's race laps; driver identity was not included as a predictive feature,
as the study treats the circuit itself as the domain unit of analysis~\cite{ref_adatime}.
...
Raw telemetry yields ten direct measurements: speed, RPM, gear (\texttt{nGear}),
throttle, brake, GPS coordinates $(x, y, z)$, elevation delta, and cumulative lap
distance — yielding $F = 12$ features after appending two physics-derived quantities:
\begin{equation}
...
$m = 798\,\text{kg}$ is the 2026 FIA minimum car mass~\cite{ref_fia2026}.
...
\begin{figure*}[t]
  ...
  \caption{Proposed two-phase system architecture. Phase~1 (top) performs anomaly
    ...
    applies an eight-model causal prediction pipeline evaluated under
...
\textbf{RF-instant}: Random Forest (200 trees) applied to the same single frame.
...
\begin{figure}[t]
  ...
  \caption{End-to-end methodology workflow. Raw 10\,Hz telemetry (FastF1) is
    ...
    Suzuka. Phase~2 evaluates eight models under LOCO with Integrated Gradients
    interpretability to diagnose cross-circuit failure modes.}
  \label{fig:workflow}
\end{figure}
...
\begin{flushleft}
\footnotesize
$^\dagger$~Persistence: $\hat{y}_{t+H} = y_t$ (current-frame mode as prediction).
Not a trained model — included as trivial lower bound.
\end{flushleft}
\end{table}
...
\begin{figure}[t]
  ...
  \caption{Mean AUC-ROC at $H{=}10$ for all eight models with $\pm 1\sigma$ error bars
...
\textbf{XGBoost and Persistence Baselines.}
To close the benchmark gap identified in earlier work, we evaluate two additional
baselines: XGBoost-instant (XGBClassifier on the last window frame, same features as
LR-instant) and Persistence ($\hat{y}_{t+H} = y_t$, the naive no-change predictor).
Results are reported in Table~\ref{tab:p2main}.
...
The persistence baseline achieves mean AUC\,=\,$0.814 \pm 0.042$, establishing a
clear lower bound that all trained models surpass; the gap ($\Delta$AUC $\approx 0.149$)
demonstrates that the learned soft boundary near the switching threshold provides
genuine anticipatory value beyond trivial mode continuity.

\textbf{Feature Ablation: GPS Coordinates.}
We also evaluated LR-instant and RF-instant with absolute position coordinates
(X, Y, Z) removed (NO-POS condition, 9 features).
Mean AUC across LOCO folds was $0.969 \pm 0.011$ for LR (FULL: $0.963$, $\Delta = +0.006$)
and $0.969 \pm 0.009$ for RF (FULL: $0.959$, $\Delta = +0.010$).
Removing GPS features \emph{improved} performance marginally, confirming that position
coordinates contributed noise rather than signal — results are entirely physics-driven
by speed, gear, and inertial features, with no reliance on circuit-identity leakage
...
Table~\ref{tab:horizon} shows mean AUC-ROC at five horizons.
...
LR-instant is statistically indistinguishable from the other instantaneous/tree models
(RF-instant, XGBoost-instant, RF-lag: $\Delta$AUC\,$\leq$\,0.019, intervals at or
spanning zero), but its advantage over every deep sequence model is consistent and its
...
\midrule
LR vs RF-instant     & $+$0.003 & [$-$0.017, $+$0.023] & No  & 2/4 \\
LR vs XGBoost-inst.  & $+$0.005 & [$+$0.001, $+$0.009] & Marg. & 3/4 \\
LR vs RF-lag         & $+$0.019 & [$-$0.010, $+$0.056] & No  & 3/4 \\
LR vs CausalTrans.   & $+$0.036 & [$+$0.009, $+$0.063] & \textbf{Yes} & 4/4 \\
LR vs CausalGRU      & $+$0.050 & [$+$0.004, $+$0.095] & \textbf{Yes} & 3/4 \\
LR vs CausalLSTM     & $+$0.085 & [$+$0.024, $+$0.151] & \textbf{Yes} & 3/4 \\
LR vs TCN            & $+$0.086 & [$+$0.014, $+$0.203] & \textbf{Yes} & 4/4 \\
LR vs CNN1D          & $+$0.166 & [$+$0.032, $+$0.346] & \textbf{Yes} & 4/4 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize
``Folds'' = number of the four held-out circuits on which LR-instant outranks the model.
...
\textcolor{white}{\textbf{Timing}} &
\textcolor{white}{\textbf{FalseTog}} &
\textcolor{white}{\textbf{MCC}} &
\textcolor{white}{\textbf{Brier}} \\
\midrule
\rowcolor{rowhighlight}
LR-instant      & \textbf{0.76} & $+$1.1 & 0.92 & \textbf{0.82} & \textbf{0.064} \\
XGBoost-instant & 0.68 & $+$1.5 & \textbf{0.82} & 0.79 & 0.069 \\
RF-instant      & 0.69 & $+$1.3 & 1.42 & 0.73 & 0.073 \\
RF-lag          & 0.58 & $+$1.1 & 1.27 & 0.62 & 0.097 \\
\midrule
Persistence     & 0.08 & $-$0.4 & 0.91 & 0.64 & 0.144 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize
Detect = fraction of true switches predicted within $\pm$0.5\,s;
Timing = mean frames between true and predicted switch ($<0$ = early);
FalseTog = spurious switches per true switch; MCC, Brier on full test set.
\end{flushleft}
\end{table}
...
XGBoost-instant attains the lowest false-toggle rate (0.82). These results confirm that
the instantaneous-model advantage is not a framewise-averaging artefact but holds on the
metrics most relevant to actuation.
...
the following for active-aero control: (1) LR-instant's 13-parameter model
could be implemented on an embedded microcontroller with microsecond latency,
potentially within the actuation budget; (2) the 1-second anticipation horizon
appears sufficient for wing actuation; (3) the consistent transition-window drop
indicates that a hysteresis rule suppressing predictions near mode-switch boundaries
may reduce unnecessary actuations.
Hardware validation and simulator testing are needed before drawing engineering
conclusions.
...
\begin{itemize}
  ...
  \item \textbf{HP tuning on a LOCO test fold.} Deep-model hyperparameters were
        selected on a Monza validation split. Because Monza is one of the four LOCO
        test circuits, this constitutes partial target-domain exposure for that fold.
        A nested LOCO procedure (tuning on the held-in circuits only) would
  ...
  \item \textbf{Position-feature circuit identity leakage.} Features derived from
        absolute track position (X, Y, Z GPS coordinates) could in principle encode
        circuit identity.
        A feature ablation (§\ref{sec:results}, NO-POS condition) showed that
        removing X, Y, Z \emph{improved} LR AUC by $+0.006$ and RF AUC by $+0.010$,
        confirming that GPS coordinates contribute noise rather than signal and that
        results are fully attributable to physics-based kinematics features.
  ...
  \item \textbf{LightGBM baseline.} XGBoost was evaluated and reported (AUC\,=\,0.961,
        matching LR within confidence intervals). LightGBM was not evaluated;
...
Phase~2 conducted, to our knowledge, the first LOCO evaluation of anticipatory aerodynamic
proxy-state prediction across four architecturally diverse circuits, comparing eight models
ranging from Logistic Regression to CausalTransformer.
...
These findings suggest — pending hardware validation and simulator testing — that
control systems — if a validated active-aero proxy label can be confirmed — should
rely on instantaneous sensor state and should incorporate transition-window hysteresis
to manage the consistent $\approx$0.26 F1 drop observed at mode-switch boundaries.

More broadly, this study demonstrates that when the prediction target is a
deterministic function of instantaneous physical quantities, \textbf{domain-invariant
feature selection is more effective than post-hoc domain adaptation}: choosing features
whose relationship to the target is circuit-independent (speed and gear) outperforms
applying adversarial or recurrent domain adaptation to features that inadvertently
encode circuit-specific dynamics.
...
reported in this paper are publicly available at:
\url{https://github.com/anasnorani1/Predictive-AI-for-Active-Aerodynamics-Efficiency-and-Driver-Anomaly-Analysis}.
