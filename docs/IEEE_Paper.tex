...
% ── System architecture figure (placed early so the full-page float lands up front) ──
\begin{figure*}[tp]
  \centering
  \includegraphics[width=\textwidth,height=0.92\textheight,keepaspectratio]{figures/architechture.png}
  \caption{Overall architecture of the proposed telemetry-derived active-aero proxy
    forecasting framework. Phase~1 (top) acquires and cleans FastF1 telemetry, engineers
    physics features, constructs the binary straight-mode proxy label, and runs discovery
    analysis. Phase~2 (bottom) formulates one-second-ahead causal forecasting
    ($W{=}50,\,H{=}10$) and compares instantaneous baselines, tree models, and deep
    sequence models under Leave-One-Circuit-Out evaluation, with robustness checks,
    event-level metrics, and Integrated Gradients. The synthesis shows instantaneous,
    circuit-invariant features generalise better than accumulated temporal context for
    this proxy task.}
...
% Note: the system architecture figure (Fig.~\ref{fig:architecture}) is defined near the
% start of Section~I so that, as a full-page double-column float, it lands on an early page.
...
% Note: the end-to-end workflow figure was merged into the system architecture
% figure (Fig.~\ref{fig:architecture}) to remove redundancy and save space.
